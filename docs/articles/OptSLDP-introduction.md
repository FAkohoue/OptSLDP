...
Genomic selection has transformed plant and animal breeding by enabling
accurate prediction of complex trait performance from dense marker
panels, without phenotyping every individual in every environment. The
quality of the marker panel used to train genomic prediction models
directly determines how much genetic signal is captured — and therefore
how accurate predictions are.

#### The marker panel construction problem

Most practitioners apply standard LD pruning (e.g., PLINK
`--indep-pairwise`) to reduce redundancy before training genomic models,
treating it as a preprocessing step rather than a modelling decision.
This approach has a fundamental flaw: it removes one marker from every
highly correlated pair **without regard to whether either marker is
statistically associated with the trait of interest**. The consequences
are:

- When a QTL region contains a cluster of correlated markers, all but
  one are discarded. The retained singleton may not be the best proxy
  for the causal variant.
- Statistical evidence about the region is reduced — markers that could
  collectively explain QTL variance are pruned to a single imperfect
  representative.
- Genomic prediction models lose signal around important loci,
  systematically underestimating the genetic variance explained by those
  regions.

#### Why SLDP improves genomic prediction

The SLDP approach (Zhu et al., 2023) addresses this by partitioning the
marker set before pruning. Regions with statistical evidence of trait
association are **protected in full**; only the remaining background
markers receive LD pruning. Zhu et al. demonstrated substantial
improvements in genomic prediction accuracy for growth and carcass
traits in pigs compared to standard uniform LD pruning.

#### Why a new implementation is needed

Modern breeding programs face three challenges beyond the original SLDP
scope:

**Scale.** Whole-genome sequencing panels routinely contain 2–10 million
SNPs across hundreds of samples. Loading the full genotype matrix into
RAM is infeasible on most workstations. `OptSLDP` handles such panels
via a disk-backed GDS backend that streams genotypes one chromosome at a
time, keeping peak RAM around 300 MB regardless of total panel size.

**Multiple traits.** Breeding programs select simultaneously for yield,
disease resistance, quality, and adaptation. A marker important for
blast resistance but not yield should not be pruned from a multi-trait
prediction panel. `OptSLDP` screens each trait independently and
protects the **union** of all per-trait candidate regions — every
genomic locus important for *any* target trait is retained.

**Population structure.** Diverse panels (multi-country nurseries,
multi-breed populations) require stratification correction to avoid
false positives that would inflate the protected set. `OptSLDP` computes
principal components automatically from the genotype data after MAF
filtering via GRM eigendecomposition, with no manual pre-computation or
...
the `alleles` column. The format has 11 metadata columns followed by
sample columns:
...
where \\y^\*\\ is the population-structure-adjusted phenotype, \\g_i\\
is the dosage vector for SNP \\i\\, and \\\alpha\\, \\\beta_i\\ are
intercept and effect size.

#### Why GLM + PCA for panel construction?

The SLDP screening step has a fundamentally different objective from a
standard GWAS. Its purpose is to identify which genomic regions carry
relevant biological signal so that all markers in those regions can be
**protected from LD pruning**. This distinction changes which
statistical properties matter:

**False positives are tolerable.** A false-positive candidate SNP causes
its surrounding \\\pm w\\ kb window to be retained in the panel. This
adds a modest number of extra markers but does not corrupt the panel or
bias downstream genomic prediction models. The final panel size
increases slightly but all true signal is preserved.
...
permanently lost from the panel. GLM + PCA controls gross inflation from
population stratification without over-correcting and suppressing true
biological signal.
...
Mixed model approaches (EMMA, EMMAX, GCTA-MLMA) are statistically more
powerful for fine-mapping but add \\O(n^3)\\ computational cost from the
...
strong population structure, increasing `n_pcs` from 3 to 5 is usually
sufficient.

#### Population structure correction

When covariates \\X_c\\ (PC scores) are supplied, the phenotype is
residualised once before the SNP loop:

\\y^\* = M\_{X_c}\\ y = \left\[I - X_c(X_c^\top X_c)^{-1}
X_c^\top\right\] y\\

This is equivalent to fitting \\y \sim X_c + g_i\\ for every SNP but
avoids refitting covariates in each regression, giving an \\O(n_c)\\
saving over \\O(n_c \cdot p)\\. When `n_pcs > 0` and
`covar_cols = NULL`, PCs are computed automatically from a LD-pruned SNP
subset before screening begins.
...
**`P.value`** — Two-tailed \\t\\-test: \\p_i = 2\\\Pr\\\left(T\_{n-2} \>
\|z_i\|\right)\\
...
at the start of every run. The GDS file is **always rebuilt** to prevent
stale data from a previous run causing incorrect pruning results.
Chromosome labels are stored as integers (supporting `1`, `chr1`,
`Chr01` formats) so that `snpgdsLDpruning()` performs chromosome-aware
sliding-window pruning correctly. All GDS file handles are tracked in a
package-level registry (`.sldp_gds_env`) and closed automatically on
pipeline exit or on error via `on.exit(.close_all_gds_handles())`.
